    ...
    public void ChangeBlenderShapeValue(string blendShapeName, float value)
    {
        ...
        if (value > 0)
        {
            if (blenderShape.positiveIndex == -1)
                return;
            skm.SetBlendShapeWeight(blenderShape.positiveIndex, value);
            if (blenderShape.negativeIndex == -1)
                return;
            skm.SetBlendShapeWeight(blenderShape.negativeIndex, 0);
        ...
        else
        {
            if (blenderShape.negativeIndex == -1)
                return;
            skm.SetBlendShapeWeight(blenderShape.negativeIndex, -value);
            if (blenderShape.positiveIndex == -1)
                return;
            skm.SetBlendShapeWeight(blenderShape.positiveIndex, 0);
        }
    }

    public bool DoesTargerMatch()
    {
        return (skm==skinnedMeshRenderer);
    }

    public void cleanBlenderShapes()
    {
        blenderShapes.Clear();
    }

    public string[] GetBlenderShapeNames()
    {
        return blenderShapes.Keys.ToArray();
    }

    public int GetNumbers()
    {
        return blenderShapes.Count;
    }

    public BlenderShape GetBlendShape(string name)
    {
        return blenderShapes[name];
    }
}
